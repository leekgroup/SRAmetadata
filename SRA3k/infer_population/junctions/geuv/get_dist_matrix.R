    ...
    if(all(is.character(junctionFiles))) {
        theData = mclapply(junctionFiles, function(x) {
            y = read.delim(x, skip = 1, header=FALSE, 
                col.names = c("chr", "start", "end", "n_junc", "count", "strand", "start2", "end2", "v1", "v2", "v3", "v4"), 
                colClasses = c("character", "integer", "integer", "character","integer", "character",rep("NULL", 6)))
...
#            pattern="^junctions\\..*\\.bed$", full.names=TRUE)

# f         <- str_split(files, "/")
# file_list <- lapply(f, `[`, length(f[[1]])) %>% unlist()

file_list <-  scan("junctions_samp_n400.txt", what="", sep="\n") -> files
...
# Get count matrix
count_m <- junctionCount(junctionFiles = paste0(path, "/",files),
               sampleNames = ids,
               maxCores = 25)

save(count_m, file = "junctions_countmatrix_geuvadis.rda")
