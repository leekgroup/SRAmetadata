...
cut -f8 $SRAINTRONS/all_SRA_introns.tsv | $MAWK -F ',' '{maxval=$1;for(i=2;i<=NF;i++){if($i>maxval){maxval=$i;}} print "zadd maxcoverage " maxval " " NR}' $SRAINTRONS/all_SRA_introns.tsv | $PYTHON proto.py | $REDIS/redis-cli --pipe --pipe-timeout 0
