...
exclude_path <- file.path('..', 'relationship_manifest_file-sample')
exclude <- read.table(exclude_path, sep = "\t")
initial_sample <- 1:n
initial_sample <- setdiff(initial_sample, exclude[,1])
n <- length(initial_sample)
cat("Size of subset: ", n) 
